...
一个面向学生的 AI 学习伙伴，通过知识图谱、测验、间隔重复、任务栈和主动提醒培养自主学习能力。
...
### 核心
- **工具调用** — Agent 通过 26 个工具执行数据库和文件操作
- **多模型** — 支持 DeepSeek、Qwen、Anthropic、OpenAI 兼容 API，支持故障转移
- **Web 界面** — 三页面架构（登录 / 学习主界面 / 管理后台），HTTP + SSE
- **多用户认证** — 注册 / 登录，bcrypt 密码哈希，session cookie，角色（普通/管理）分离
- **流式输出** — 实时 token 输出，思考/推理过程可折叠展示
- **对话记忆** — 滑动上下文窗口（可配置）+ 会话持久化 + 上下文摘要
...
- **知识库** — 学科、知识点（多层树状层级支持），知识点间关联（前置/横向关系），别名搜索
- **RAG 语义检索** — 向量嵌入（bge-base-zh-v1.5）语义匹配，环境变量切换（FTS/LIKE 降级）
- **错题追踪** — 自动记录错题，SM-2 间隔重复（1→3→7→14→30 天）
- **掌握度评估** — EWMA 算法追踪每个知识点的掌握度（0~1），知识图谱节点动态着色
- **AI 错题归因** — 分析错误答案，定位最可能薄弱的关联知识点
- **学习计划（1号计划）** — AI 摸底评估 → 制定分章计划 → 嵌套测验 → 自动衔接下一章
- **定时提醒** — 每日复习检查、考试倒计时、计划提醒
- **数据导入导出** — 批量 JSON/CSV 导入题目和知识点，错题导出

### 架构
- **AQC 自适应查询缓存** — 高频自然语言查询本地匹配，双层 LRU 缓存，定期清理
- **__CARD__ 指令通道** — 画布操作统一入口（知识点卡片、创建测验、工具栏），绕过缓存和 LLM
- **任务栈（TaskEngine）** — 任务模式管理、阶段追踪、任务守卫
- **持久 SSE 连接** — 系统事件推送（测验弹窗、模式变更、进度通知），自动断线重连

### Web 界面
- **登录/注册页** — 用户认证入口
- **知识图谱画布** — 拖拽/缩放/聚焦，DOM 节点 + SVG 连线，左上角布局，缩放一致性，节点颜色绑定掌握度
- **聊天面板** — 实时流式输出，LaTeX/KaTeX 渲染，Markdown 渲染，思考过程可折叠
- **测验弹窗** — 点击知识点卡片或 Agent 指令触发，支持批量提交，即时批改显示正误
- **管理后台** — 学科/知识点/题目 CRUD，用户管理，去重检测，数据导入
- **底部工具栏** — 1号计划进度、知识强化、错题测试、默认布局、设置
...
domiclaw/
├── src/
│   ├── index.ts              # 入口：初始化数据库 + 启动 Web 服务 + 定时任务
│   ├── db.ts                 # SQLite — 14+ 表，60+ 查询函数
│   ├── agent.ts              # 重新导出 src/agent/
│   ├── agent/
│   │   ├── index.ts          #   入口
│   │   ├── loop.ts           #   工具调用循环（带守卫、阶段回调）
│   │   ├── environment.ts    #   系统提示构建（RAG 注入 + 会话上下文 + 硬约束规则）
│   │   └── model.ts          #   LLM API 调用（流式 / 非流式）
│   ├── task/
│   │   └── taskEngine.ts     #   任务栈管理（任务推断、守卫、阶段追踪）
│   ├── query-router.ts       #   自适应查询缓存（AQC）+ __CARD__ 指令通道
│   ├── auth.ts               #   用户认证（注册/登录/session管理）
│   ├── config.ts             #   配置（从 .env 加载）
│   ├── sessionStore.ts       #   自定义 better-sqlite3 session 存储
│   ├── sseBus.ts             #   SSE 事件总线（系统事件 + 流式响应）
...
│   │   └── retrievers/
│   │       ├── base.ts       #     抽象接口
│   │       ├── ftsRetriever.ts #     LIKE/FTS 降级检索
│   │       └── vectorRetriever.ts # 向量语义检索
│   ├── channels/
│   │   └── http.ts           #   Express + SSE + REST API + 静态文件
│   └── tools/
│       ├── index.ts          #   工具注册表 + 注册/查询函数
│       ├── utils.ts          #   共享工具函数（checkAnswer, formatOptions 等）
...
| `WEBCLIENT_PORT` | Web 界面端口（留空=仅定时任务） | （禁用） |
...
| `MAX_CONTEXT_MESSAGES` | 上下文窗口大小 | `30` |
...
| `VECTOR_SEARCH_ENABLED` | 启用向量语义检索 | `false`（= FTS 降级） |
| `EMBEDDING_MODEL` | 嵌入模型名称 | `Xenova/bge-base-zh-v1.5` |
| `RAG_TOP_K` | 检索返回条数 | `3` |
| `HF_MIRROR` | HuggingFace 镜像 | — |
| `VISION_MODEL_ENABLED` | 启用视觉模型 | `false` |
| `VISION_MODEL_NAME` | 视觉模型名称 | `MiMo-V2.5` |

## RAG 配置

向量检索默认关闭（使用 FTS/LIKE 降级检索），开启后首次启动会下载模型（~103MB）：
...
用户消息
  ...
  → 加载对话上下文（最近 N 条消息，防截断保护）
  → 构建系统提示词（AGENT.md 角色定义 + RAG 检索注入 + 会话上下文 + 任务栈 + 硬约束规则）
  → 如为定时检查：附加 Scheduled Check-in 前缀
  → 调用 LLM（带 26 个工具定义，首轮流式输出）
  → 如有工具调用：本地执行（带任务守卫），结果反馈给模型，最多 10 次循环
...
```
每次查询先过 AQC：
  1. __CARD__: 开头 → 直接拦截，执行工具，返回结果
  2. 缓存命中 → 直接返回缓存的 LLM 回复
  3. 缓存未命中 → 调用 LLM 做意图分类（无需调用工具的问题）
     - chat → 放行给 Agent
     - stats/wrong/review/quiz → 有对应工具路由
  4. 新回复写入缓存（LRU，上限 100 条用户级 / 500 条系统级，定期清理）
```
...
```
任务类型：
  quiz       — 测验任务（带任务守卫，禁止多测验并行）
  self_eval  — 自我评估任务（支持嵌套测验子任务）
  review     — 复习任务

任务生命周期：
  启动 → 进行中（阶段：before / during / after）→ 结束
  任务守卫：相同类型任务并行时拒绝新任务
  嵌套：self_eval 可嵌套 quiz 子任务
```
